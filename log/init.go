...
func NewLogger(conf *config.Config) *zap.Logger {
	_, err := os.Stat(conf.Ink.LogDir)
	fmt.Println(config.LogToConsole())
	if err != nil {
		if os.IsNotExist(err) && !config.LogToConsole() {
			err := os.MkdirAll(conf.Ink.LogDir, os.ModePerm)
			if err != nil {
...
func getWriter(conf *config.Config) zapcore.WriteSyncer {
	rotatingLogger := &lumberjack.Logger{
		Filename: filepath.Join(conf.Ink.LogDir, conf.Log.FileName),
